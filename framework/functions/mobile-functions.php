...
if( ! function_exists( 'tie_mobile_header_components' ) ) {
	...
	function tie_mobile_header_components( $area ){
		...
		// If Google Search is active, disable the popup search
	//	if( tie_get_option( 'google_search_engine_id' ) ){
		//	unset( $mobile_header_components['search'] );
		//}
		...
		if( is_user_logged_in() ){
			...
			if( get_option( 'show_avatars' ) ){
				...
				$login_icon = get_avatar( $current_user->ID, apply_filters( 'TieLabs/Login/avatar_size', 30 ) );
			}
			else{
				$login_icon = '<span class="tie-icon-author" aria-hidden="true"></span>';
			}

			$login_icon .= '<span class="screen-reader-text">'. esc_html__( 'Your Profile', TIELABS_TEXTDOMAIN ) .'</span>';

			$mobile_header_components['login'] = '
				<a href="#" class="profile-btn">'. $login_icon .'</a>
