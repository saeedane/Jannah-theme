...
if( ! class_exists( 'TIELABS_FOXPUSH' ) ) {
	...
	class TIELABS_FOXPUSH{
		 ...
		 * FoxPush Statistics
		 */
		function get_statistics( $type = 'daily_chart' ){
			...
			// Get stored data
			$data = get_transient( 'tie_foxpush_'. $type );
			...
			if( empty( $data ) ) {
				...
				$api_url = 'https://api.foxpush.com/v1/publisher/' . $type;
				$request = wp_remote_get( $api_url, $args );
				
				// Cache errors
				if ( is_wp_error( $request ) || ( ! empty( $request->errors ) && isset( $request->errors['http_request_failed'] ) ) || ( 200 !==  wp_remote_retrieve_response_code( $request ) ) ){
					$data = 'error';
					$cache_time = 24;
				}
				else{
				...
				set_transient( 'tie_foxpush_'. $type, $data, $cache_time * HOUR_IN_SECONDS );
			}

			return ( ! empty( $data ) && $data != 'error' ) ? $data : '';
		}
