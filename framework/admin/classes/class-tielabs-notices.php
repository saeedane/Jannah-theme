	...
	class TIELABS_NOTICES {
		...
		function load_notices(){
			...
			if ( $theme_pages == 'toplevel_page_tie-theme-options' ){
				...
				if( ! self::is_dismissed( 'tie_settings_page_'. TIELABS_THEME_SLUG ) ){

					add_action( 'admin_print_footer_scripts', array( $this, 'settings_page' ) );
					wp_enqueue_style ( 'wp-pointer' );
					wp_enqueue_script( 'wp-pointer' );
				}
			}
		...
		function settings_page(){
			$pointer_content  = '<h3>'. esc_html__( 'Hi, Are you looking for me?', TIELABS_TEXTDOMAIN ) .'</h3>';
			$pointer_content .= '<p style="font-size: 14px;">'. esc_html__( 'You can import demos, install the bundled plugins, and manage theme settings from here!', TIELABS_TEXTDOMAIN ) .'</p>'; ?>

			<script>
				//<![CDATA[
				jQuery(document).ready( function($){
					$('#toplevel_page_tie-theme-options').pointer({
						content: '<?php echo wp_kses_post( $pointer_content ); ?>',
						pointerWidth:	350,
						position: {
							edge : 'left',
						...
						close: function(){
							$.post( ajaxurl, {
								pointer: 'tie_settings_page_<?php echo esc_js( TIELABS_THEME_SLUG ) ?>',
								action : 'dismiss-wp-pointer',
							});
						}
					}).pointer('open');
