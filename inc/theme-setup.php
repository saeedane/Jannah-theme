...
function jannah_register_assets(){
	...
	if( tie_get_option( 'google_search_engine_id' ) ){
		wp_register_script( 'tie-google-search', 'https://cse.google.com/cse.js?cx='. TIELABS_HELPER::remove_spaces( tie_get_option( 'google_search_engine_id' ) ), array(), 1, true );
	}
 ...
 */
 ...
 function jannah_plugins_installer_data( $plugins_data = false ){
 	...
 	if( ! empty( $plugins_data ) || tie_get_token() ){
 		return $plugins_data;
 	}
